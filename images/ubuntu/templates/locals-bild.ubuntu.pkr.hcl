...
# --- 1. Variables (Required for the scripts) ---
variable "image_folder" {
  default = "/opt/runner-images/images/ubuntu"
}
variable "helper_script_folder" {
  default = "/opt/runner-images/images/ubuntu/scripts/helpers"
}
variable "installer_script_folder" {
  default = "/opt/runner-images/images/ubuntu/scripts/build"
}
variable "image_os" {
  default = "ubuntu2404"
}
...
variable "imagedata_file" {
  default = "/opt/runner-images/images/ubuntu/image-data.json"
}
...
source "null" "local" {
  communicator = "ssh"
  ssh_host     = "127.0.0.1"
  ssh_username = "debian"  # CHANGE THIS to your current username
  ssh_password = "debian" # CHANGE THIS to your current password
}
...
build {
  ...
  # === YOUR ORIGINAL PROVISIONERS START HERE ===
  ...
  # NOTE: Reboot is risky in local builds but required for kernel updates.
  # If Packer hangs here, you may need to reconnect manually.
  provisioner "shell" {
    execute_command       = "sudo sh -c '{{ .Vars }} {{ .Path }}'"
    expect_disconnect     = true
    inline                = ["echo 'Rebooting VM for updates...'", "sudo reboot"]
  }
  ...
  # --- MODIFIED: REMOVED Azure Waagent Deprovisioning ---
  # We replaced the original Azure-specific deprovision command with a simple log cleanup.
  provisioner "shell" {
    execute_command = "sudo sh -c '{{ .Vars }} {{ .Path }}'"
    inline          = [
